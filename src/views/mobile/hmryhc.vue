<template>
  <div>
    ...
    >
      ...
      <div class="down-link" @click="hmryhcExportData">点击下载核查明细表</div>
      ...
      <el-collapse v-model="activeName" class="card">
        ...
        >
          <div class="person-card" :model="data.personnelData[index]">
            ...
            <el-form-item class="line">
              <div class="title" slot="label">电话</div>
              <el-input :value="item.sjhm" disabled />
            </el-form-item>
            <el-form-item class="line">
              <div class="title" slot="label">身份证号</div>
              <el-input :value="item.sfzhm" disabled />
            </el-form-item>
            <el-form-item class="line">
              <div class="title" slot="label">地址</div>
...
// import {hmryhcExportData} from '@/api/alarm'
export default {
  name: "hmryhc",
  components:{
    ImportFile
  ...
  methods: {
    ...
    //导出
    async hmryhcExportData() {
      await hmryhcExportData(this.token)
    },
    bzPlaceHolder(item) {
      if (item.qxdl === "无法落地") {
        return "填写原因";
