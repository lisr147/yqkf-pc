<template>
  <div class="big-container screen-wrap">
    <!-- 2022-1-24 增加核酸检测、疫苗接种两个图层，流调数据总览修改 @huangguangcong -->
    <screen-header @layer="switchLayer" :layer="layer" />

    <base-map-layer
      :layerIds="layerIds"
      ref="baseMapLayer"
      @layersReady="handleLayerReady"
    ...
    <!--一图总览-->
    <AllDataTotal v-if="layer === 20" @close="layer = 0" />
    ...
    <MobileDialog
      ...
      @close="layer = 0"
    />
    <!-- 6, 7, 8...为图层ID,记录于 src/components/screen/ScreenHeader.vue -->

    <!--总数统计-->
    <div class="total-btn-container">
      <template v-if="visibleIds.includes('redYellow')">
        <div class="total-btn-wrapper act" @click="showTotalDetail('red')">
          <span class="t1">红码人员</span>
          <span class="t2" style="color: #e8473f"
            >{{ amountData.redTotal }} ( 本地{{
              amountData.redTotal - amountData.redOther
            }}
            其他{{ amountData.redOther }} )</span
          >
          <el-popover popper-class="screen-popover" width="360" trigger="hover">
            <div>
              本地数：是南沙区3街6镇的红码人数之和，其他数：是镇街为“其他”的红码人数之和
            </div>
            <i slot="reference" class="el-icon-question"></i>
          </el-popover>
        </div>
        <div class="total-btn-wrapper act" @click="showTotalDetail('yellow')">
          <span class="t1">黄码人员</span>
          <span class="t2"
            >{{ amountData.yellowTotal }} ( 本地{{
              amountData.yellowTotal - amountData.yellowOther
            }}
            其他{{ amountData.yellowOther }} )
          </span>
          <el-popover width="360" popper-class="screen-popover" trigger="hover">
            <div>
              本地数：是南沙区3街6镇的黄码人数之和，其他数：是镇街为“其他”的黄码人数之和
            </div>
            <i slot="reference" class="el-icon-question"></i>
          </el-popover>
        </div>
      ...
      <total-toast
        ...
        :key="toastItem.name"
        @click.native="showTotalDetail(toastItem.name, toastItem.total)"
        :toastItem="toastItem"
      >
      </total-toast>
    </div>
    ...
    <map-layer-filter
      ...
      :hasSealLink="
        $store.state.authority.accessPaths.includes('sealareaIndex')
      "
    ></map-layer-filter>
    ...
    >
      <template v-slot:right-3>
        <div>
          <div v-for="(comp, index) in linkData" :key="index">
            <data-board
              from="indexRight"
              v-if="comp.code && comp.chart !== 'block'"
              :about="comp.about"
              :id="comp.chartId"
              :code="comp.code"
              :title="comp.title"
            ...
            <chart-party
              v-if="comp.id === 'party'"
              @showUnitSupportStreet="showUnitSupportStreet"
            ></chart-party>
            <SamplingPoint
              v-if="comp.id === 'detectionPoint'"
              @showSamplingPointList="showSamplingPointList"
            ></SamplingPoint>
            <key-place v-if="comp.id === 'keyPlaces'"></key-place>
            <block-statistics
              from="indexRight"
              v-if="comp.code && comp.chart === 'block'"
              :about="comp.about"
            ...
            ></block-statistics>
            <risk-count
              v-if="comp.id === 'dangerPeople'"
              from="right"
              @showMhRiskDetail="showMhRiskDetail"
            ></risk-count>
            <medical v-if="comp.id === 'hospital'" @showDialog="showDialog" />
          </div>
        </div>
      </template>
    </report-layer>
    ...
    <map-legend
      ...
      @villageLayerSwitch="handleVillageSwitch"
      @gridLayerFilter="handleGridFilterGz2000"
      ...
      :hasAlarmCtrl="true"
      @toggleAlarm="handelToggleAlarm"
      @showLineUpMap="showLineUpMap"
      ...
      >
        <template slot="title" @click="zIndex++">
          <div>
            ...
            <div v-if="item.detailType === 'redYellow'">人员列表</div>
    ...
    ></stream>
    <!--<el-button style="position: fixed;top: 5em; left: 35em; z-index:999;" @click="handelEventAlarmMsg">模拟推送告警</el-button>-->
  </div>
</template>
...
import BaseMapLayer from "./layer/BaseMapLayerGz2000";
import MapLayerFilter from "./layer/MapLayerFilter";
...
import GridDetail from "./layer/GridDetail";
import mhRiskDetail from "@/components/screen/mhRiskDetail";
import ThreeGroupDetail from "./layer/ThreeGroupDetail";
import SpecialPeopleDetail from "./layer/SpecialPeopleDetail";
import SpecialPeopleDetailList from "./layer/SpecialPeopleDetailList";
import VideoList from "./layer/VideoList";
import VideoDetailBoard from "./layer/VideoDetailBoard";
import VideoRecordBoard from "./layer/VideoRecordBoard";
import StaffDetailList from "./layer/StaffDetailList";
import LogDetailList from "./layer/LogDetailList";
import DetailList from "./layer/DetailList";

import transitVehiclesList from "./layer/TransitVehiclesList";
import AllocationHealthCareList from "./layer/AllocationHealthCareList";
import consultationSchedulingList from "./layer/ConsultationSchedulingList";

import UnitSupportStreet from "./layer/UnitSupportStreet";
...
import HistoryList from "./layer/HistoryList";
import BigChart from "@/components/charts/BigChart";
import TownCount from "./layer/TownCount";
...
export default {
  components: {
    ...
    DetailList,
    Stream,
    medical: () => import("./medical.vue")
  },
  name: "BigScreenGz2000",
  // 父组件中返回要传给下级的数据
  provide() {
    return {
  ...
  data() {
    return {
      ...
      zIndex: 1,
      showPlaceMap: false,
      layer: 0, //当前图层: 0 默认地图， 1公安流调  3嘉安花园， 4封控管理区，5流调数据总览,6疫苗接种,7核酸检测 8数据说明,12 人员核查, 20一图总览
  ...
  computed: {
    ...
    villageOptVisible() {
      return this.$store.state.common.village !== null;
    },
    // 左上角总数toast提示
    toastItems() {
  ...
  created() {
    ...
    if (!localStorage._selected_layer_ids) {
      ...
      //   ];
      this.linkData = [
        { label: "医院", id: "hospital", checked: true, order: 2, isLink: true }
      ];
    } else {
  ...
  methods: {
    ...
    async initAmount() {
        ...
        },
        {
          ...
          data: {
            total: nucleicCount.detail.length
              ? nucleicCount.detail.find(item => item.key === "常规核酸检测点")
                  ?.value
              : 0
          }
        },
      ...
      //黄码人员统计
      //   黄码不用公安数据
      //   getStreetYellowCodeStatistics({ sszj: getCurrTown() }).then((res) => {
      //     const { yellowTotal, yellowQt } = res;
      //     this.amountData.yellowTotal = yellowTotal;
      //     this.amountData.yellowOther = yellowQt;
      //   });
    },
    ...
    handleLayerReady() {
      ...
      //初始化告警图标
      this.$refs["baseMapLayer"].addLayer({
        id: "event-alarm",
        visible: false
      });
    ...
    async handelLayerClicked({ layerId, attributes, event, target }) {
      ...
      const hideSelectLayer = () => {
        const village = this.$refs["baseMapLayer"].map.findLayerById(
          "village-gz2000"
        );
        if (village) {
          village.visible = false;
        }
        const street = this.$refs["baseMapLayer"].map.findLayerById(
          "street-gz2000"
        );
        if (street) {
          street.visible = false;
        }
        const grid = this.$refs["baseMapLayer"].map.findLayerById(
          "grid-gz2000"
        );
        if (grid) {
          grid.visible = false;
      ...
      switch (layerId) {
        case "street-gz2000":
          ...
          this.currGrid = "";
          //开启遮罩下钻
          this.$refs["baseMapLayer"].maskToGz2000(target.graphic);
          //镇街图层相关统计
          this.initAmount();
          hideSelectLayer();
          break;
        case "grid-gz2000":
          ...
          break;
        case "village-gz2000":
          this.$store.commit("common/SET_VILLAGE", attributes.name);
          this.currVillage = attributes.name;
          //开启遮罩下钻
          this.$refs["baseMapLayer"].maskToGz2000(target.graphic);
          //   过滤村居
          //   this.$refs["baseMapLayer"].filterDataInLayer({
          //     id: "village-gz2000",
          //     filterCode: `name LIKE '%${attributes.name}%'`
          //   });
          hideSelectLayer();
    ...
    async backToWhole() {
      ...
      if (village) {
        village.visible = this.$refs.mapLegend.isVillageVisible;
      }
      const street = this.$refs["baseMapLayer"].map.findLayerById(
        "street-gz2000"
      );
      if (street) {
        street.visible = !this.$refs.mapLegend.isGridVisible;
      }
      const grid = this.$refs["baseMapLayer"].map.findLayerById("grid-gz2000");
      if (grid) {
        grid.visible = this.$refs.mapLegend.isGridVisible;
      }

      await this.$refs["baseMapLayer"].backToWholeFromMaskGz2000();
      this.$store.commit("common/SET_GRID", null);
      this.$store.commit("common/SET_TOWN", null);
      this.$store.commit("common/SET_VILLAGE", null);
      this.currTown = null;
    ...
    closeDetailList(key) {
      ...
      this.$set(this.detailList, key, null);
    },
    ...
    showDialog(dialogData) {
      console.log("showDialog", dialogData);
      //   this.detailList[dialogData.dialogId] = dialogData;
      this.$set(this.detailList, dialogData.dialogId, dialogData);
    }
  },
  watch: {
    currTown(val) {
      ...
      try {
        ...
        _this
          ...
          .then(() => {
            ...
            window.open(
              `https://dnvideopre.iot-cas.com/video/index.html#/?name=${_this.$store.state.user.username}&romm=999`
            );
          })
          .catch(() => {});
      } catch (e) {
...
.el-button:focus, .el-button:hover {
    color: #fff;
    border-color: rgba($color: #1a449c, $alpha: .8);
    background-color: rgba($color: #409eff, $alpha: .8);
}
</style>
