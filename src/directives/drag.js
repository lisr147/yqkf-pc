...
Vue.directive('dialogDrag', {
  async bind (el, binding, vnode, oldVnode) {
    ...
    const dragDom = el.querySelector('.el-dialog') || el
    const zIndexDom = dragDom.parentNode || el
    dialogHeaderEl.style.cursor = 'move'
    let zIndex = await store.dispatch('common/getDialogZIndex')
    zIndexDom.style.zIndex = zIndex

    // 获取原有属性 ie dom元素.currentStyle 火狐谷歌 window.getComputedStyle(dom元素, null);
    const sty = dragDom.currentStyle || window.getComputedStyle(dragDom, null)

    // 鼠标按下事件
    dialogHeaderEl.onmousedown = async (e) => {
      let nextZIndex = await store.dispatch('common/getDialogZIndex')
      zIndexDom.style.zIndex = nextZIndex
      ...
      document.onmousemove = function (e) {
        ...
        dragDom.style.top = `${Math.max(85,finallyT)}px`
