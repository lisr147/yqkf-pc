        ...
        >
          ...
          <el-row :gutter="20">
            <slot
              ...
              name="form-content"
            >
              <template v-for="(formItem, field, index) of orderedFormDesc">
                <slot
                  :name="field + '-wrapper'"
                  :data="formData[field]"
                  :desc="formItem"
...
<script>
  import responsiveMixin from 'vue-ele-form/lib/mixins/responsiveMixin'
  import { isUnDef, is, castArray, isEmpty } from 'vue-ele-form/lib/tools/utils'
  import { throttle } from 'throttle-debounce'
  import localeMixin from 'vue-ele-form/lib/mixins/locale'
  import { t } from 'vue-ele-form/lib/locale'
  import { loadMockJs } from 'vue-ele-form/lib/tools/mock'
  ...
  export default {
    ...
    computed: {
      ...
      orderedFormDesc() {
        console.log('orderedFormDesc order' + this.order)
            ...
            this.$refs.form.validate(async (valid, invalidFields) => {
              if (valid) {
                ...
                for (const key in invalidFields) {
                  await this.$notify({message: invalidFields[key][0].message, title: '', type: 'warning'})
                }
