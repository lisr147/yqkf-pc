  ...
  watch: {
    config: {
      handler(config) {
        ...
        // console.log('排序',formConfig)
        if(!formConfig.formDesc){
          return
        }
        this.formItemList = []
        formConfig.order.forEach(v=>{
          formConfig.formDesc[v].field = v
          this.formItemList.push(formConfig.formDesc[v])
        })
        this.formItemList = this.formItemList.map(item =>
